...
const ShoppingProfilesPage = observer(({ isAuthenticated = true }) => {
  ...
  const handleEditProfile = () => {
    const profileToEdit = profiles.find(profile => profile.id === selectedProfileId);
    ...
    setFormData({
      ...
      interests: profileToEdit.interests.join(', '),
      stylePreferences: profileToEdit.stylePreferences.join(', '),
      favoriteColors: profileToEdit.favoriteColors.join(', '),
      favoriteCategories: profileToEdit.favoriteCategories.join(', '),
  ...
  const handleCreateProfileSubmit = () => {
    ...
    const newProfile = {
      ...
      stylePreferences: formData.stylePreferences.split(',').map(item => item.trim()),
      favoriteColors: formData.favoriteColors.split(',').map(item => item.trim()),
      favoriteCategories: formData.favoriteCategories.split(',').map(item => item.trim()),
      avatar: formData.avatar || `https://randomuser.me/api/portraits/${formData.gender.toLowerCase() === 'female' ? 'women' : 'men'}/${Math.floor(Math.random() * 100)}.jpg`,
  ...
  const handleAiProfileGeneration = () => {
    ...
    setTimeout(() => {
      ...
      const aiGeneratedProfile = {
        name: aiPrompt.includes('husband') ? 'David (Husband)' : 
              aiPrompt.includes('daughter') ? 'Lily (Daughter)' : 
              aiPrompt.includes('son') ? 'Ethan (Son)' : 
              'New AI Profile',
        relationship: aiPrompt.includes('husband') || aiPrompt.includes('wife') ? 'spouse' : 
                      aiPrompt.includes('daughter') || aiPrompt.includes('son') ? 'child' : 
                      aiPrompt.includes('mother') || aiPrompt.includes('father') ? 'parent' : 
                      'other',
        age: aiPrompt.includes('husband') || aiPrompt.includes('wife') ? 35 : 
             aiPrompt.includes('daughter') || aiPrompt.includes('son') ? 10 : 
             aiPrompt.includes('mother') || aiPrompt.includes('father') ? 65 : 
             30,
        gender: aiPrompt.includes('husband') || aiPrompt.includes('son') || aiPrompt.includes('father') ? 'Male' : 'Female',
        interests: aiPrompt.includes('tech') ? ['Technology', 'Gadgets', 'Smart home'] : 
                  aiPrompt.includes('fashion') ? ['Fashion', 'Style', 'Trends'] : 
                  aiPrompt.includes('sport') ? ['Sports', 'Fitness', 'Outdoor activities'] : 
                  ['Reading', 'Movies', 'Travel'],
        stylePreferences: aiPrompt.includes('casual') ? ['Casual', 'Comfortable', 'Relaxed'] : 
                          aiPrompt.includes('formal') ? ['Formal', 'Business', 'Elegant'] : 
                          aiPrompt.includes('sport') ? ['Athletic', 'Sporty', 'Active'] : 
                          ['Modern', 'Classic', 'Versatile'],
        favoriteColors: aiPrompt.includes('blue') ? ['Blue', 'Navy', 'Teal'] : 
                        aiPrompt.includes('red') ? ['Red', 'Burgundy', 'Coral'] : 
                        aiPrompt.includes('green') ? ['Green', 'Olive', 'Mint'] : 
                        ['Black', 'Gray', 'White'],
        favoriteCategories: aiPrompt.includes('tech') ? ['Electronics', 'Gadgets', 'Smart home'] : 
                            aiPrompt.includes('fashion') ? ['Clothing', 'Accessories', 'Shoes'] : 
                            aiPrompt.includes('sport') ? ['Sportswear', 'Equipment', 'Outdoor gear'] : 
                            ['Books', 'Home decor', 'Kitchen'],
        avatar: `https://randomuser.me/api/portraits/${aiPrompt.includes('husband') || aiPrompt.includes('son') || aiPrompt.includes('father') ? 'men' : 'women'}/${Math.floor(Math.random() * 100)}.jpg`,
        isDefault: profiles.length === 0
      };
    ...
    <>
      ...
      <Container maxWidth="md" sx={{ mt: 4, mb: 8 }}>
        ...
        <Typography variant="body1" color="text.secondary" paragraph>
          Create and manage shopping profiles for yourself and others. Find style twins and get personalized recommendations for each profile.
        </Typography>
              ...
              <Grid item xs={12} sm={6} md={4} key={profile.id}>
                <Card 
                  sx={{ 
                    ...
                    position: 'relative',
                    border: profile.isDefault ? `2px solid ${theme.palette.primary.main}` : 'none'
                  }}
                >
                  {profile.isDefault && (
                    <Chip 
                  ...
                  <CardContent sx={{ flexGrow: 1 }}>
                    ...
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, mb: 1 }}>
                      ...
                      {profile.interests.length > 3 && (
                        <Chip label={`+${profile.interests.length - 3}`} size="small" variant="outlined" />
                      )}
                    </Box>
                    ...
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                      {profile.stylePreferences.slice(0, 3).map((style, index) => (
                        <Chip key={index} label={style} size="small" variant="outlined" />
                      ))}
                      {profile.stylePreferences.length > 3 && (
                        <Chip label={`+${profile.stylePreferences.length - 3}`} size="small" variant="outlined" />
                      )}
                    </Box>
                  </CardContent>
        ...
        >
          ...
          {selectedProfileId && !profiles.find(p => p.id === selectedProfileId)?.isDefault && (
        ...
        <Dialog open={editDialogOpen} onClose={() => setEditDialogOpen(false)} maxWidth="sm" fullWidth>
          <DialogTitle>Edit Shopping Profile</DialogTitle>
          <DialogContent>
            <Grid container spacing={2} sx={{ mt: 0.5 }}>
              <Grid item xs={12}>
                <TextField
        ...
        <Dialog open={aiDialogOpen} onClose={() => setAiDialogOpen(false)} maxWidth="sm" fullWidth>
          ...
          <DialogContent>
            <Typography variant="body2" color="text.secondary" paragraph>
              Describe the person you want to create a shopping profile for, and our AI will generate a profile based on your description.
            </Typography>
            ...
            <Box sx={{ mt: 3 }}>
              ...
              <List dense>
                <ListItem>
                  <ListItemText 
                    primary="My 7-year-old daughter who loves pink, unicorns, and art. She's into comfortable clothes and enjoys reading."
                  />
                </ListItem>
                <ListItem>
                  <ListItemText 
                    primary="My teenage son who is into sports, video games, and tech. He prefers casual sporty clothes and the color red."
                  />
                </ListItem>
                <ListItem>
                  <ListItemText 
                    primary="My mother who is 65, loves gardening and cooking. She prefers classic styles and comfortable shoes."
                  />
                </ListItem>
              </List>
            </Box>
          ...
          <DialogActions>
            ...
            <Button 
              variant="contained" 
              startIcon={aiGenerating ? <CircularProgress size={20} color="inherit" /> : <AutoAwesomeIcon />}
